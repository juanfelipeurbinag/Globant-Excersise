...
We have three variables (admission_type_id, discharge_disposition_id, admission_source_id) which are ids that need to be considered as factors instead of integer for the analysis.
...
We can observe that the classification tree at this level only classifies between NO readmittance and >30 readmittance. To get the <30 level we would need to grow the tree larger.
